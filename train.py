...
class Trainer:
	...
	def optimize(self):
		...
		if self.iter % 100 == 0:
			print('Iteration :- ', self.iter, ' Loss_actor :- ', loss_actor.data.detach().cpu().numpy(),\
				' Loss_critic :- ', loss_critic.data.detach().cpu().numpy())
		self.iter += 1
		
		return self.iter,loss_actor.data.detach().cpu().numpy() , loss_critic.data.detach().cpu().numpy()

	def save_models(self, save_dir,episode_count):
		"""
		saves the target actor and critic models
		:param episode_count: the count of episodes iterated
		:return:
		"""
		torch.save(self.target_actor.state_dict(), './' + save_dir + str(episode_count) + '_actor.pt')
		torch.save(self.target_critic.state_dict(), './' + save_dir + str(episode_count) + '_critic.pt')
	...
	def load_models(self, episode):
		...
		:return:
		"""
		self.actor.load_state_dict(torch.load('./Model_history/' + str(episode) + '_actor.pt'))
		self.critic.load_state_dict(torch.load('./Model_history/' + str(episode) + '_critic.pt'))
		utils.hard_update(self.target_actor, self.actor)
		utils.hard_update(self.target_critic, self.critic)
